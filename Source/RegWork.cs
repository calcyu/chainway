...
namespace Urbrural
{
    ...
    public class AdmlyRegWork : RegWork
    {
        ...
        public void @default(WebContext wc)
        {
            wc.GivePage(200, h =>
            {
                h.TOOLBAR(subscript: MvScope.TYP_PROV);
                using var dc = NewDbContext();
                dc.Sql("SELECT ").collst(MvScope.Empty).T(" FROM regs WHERE typ = ").T(MvScope.TYP_PROV).T(" ORDER BY id, state DESC");
                var arr = dc.Query<MvScope>();
                h.TABLE(arr, o =>
                    {
                        h.TDCHECK(o.Key);
                        h.TDAVAR(o.Key, o.name);
                        h.TDFORM(() => h.TOOLGROUPVAR(o.Key, subscript: MvScope.TYP_PROV));
                    }
                );
            });
        }
        ...
        public async Task @new(WebContext wc, int typ)
        {
            ...
            if (wc.IsGet)
            {
                wc.GivePane(200, h =>
                {
                    ...
                    h.LI_().SELECT("类型", nameof(o.typ), o.typ, MvScope.Typs, filter: (k, v) => k == typ, required: true)._LI();
